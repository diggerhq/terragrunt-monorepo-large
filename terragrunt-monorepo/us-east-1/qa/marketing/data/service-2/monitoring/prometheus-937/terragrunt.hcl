include "root" {
  path = find_in_parent_folders()
}

dependencies {
  paths = [
    "../../../../../../../../eu-west-1/staging/operations/platform/service-1/compute/ec2-41",
    "../../../../../../../../eu-west-1/dev/operations/application/service-1/application/api-gateway-694",
    "../../../../../../../../eu-west-1/qa/finance/platform/service-3/storage/dynamodb-57",
    "../../../../../../../../eu-west-1/qa/marketing/platform/service-3/storage/elasticache-912",
  ]
}

terraform {
  source = "git::git@github.com/example/terraform-modules.git//prometheus-937?ref=v1.0.0"
}

locals {
  account_vars = read_terragrunt_config(find_in_parent_folders("account.hcl"))
  region_vars  = read_terragrunt_config(find_in_parent_folders("region.hcl"))
  env_vars     = read_terragrunt_config(find_in_parent_folders("env.hcl"))
}

inputs = {
  name = "prometheus-937"
  environment = local.env_vars.locals.environment
  region = local.region_vars.locals.aws_region
  tags = {
    BusinessUnit = "engineering"
    ServiceCategory = "application"
    Environment = local.env_vars.locals.environment
    ManagedBy = "terragrunt"
  }
}