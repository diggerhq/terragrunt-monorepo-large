include "root" {
  path = find_in_parent_folders()
}

dependencies {
  paths = [
    "../../../../../../../../us-east-1/dev/marketing/data/service-2/network/transit-gateway-83",
    "../../../../../../../../us-west-2/staging/marketing/platform/service-3/storage/s3-873",
    "../../../../../../../../eu-west-1/staging/marketing/application/service-2/compute/fargate-250",
    "../../../../../../../../us-east-1/dev/finance/platform/service-2/storage/dynamodb-396",
  ]
}

terraform {
  source = "git::git@github.com/example/terraform-modules.git//alertmanager-748?ref=v1.0.0"
}

locals {
  account_vars = read_terragrunt_config(find_in_parent_folders("account.hcl"))
  region_vars  = read_terragrunt_config(find_in_parent_folders("region.hcl"))
  env_vars     = read_terragrunt_config(find_in_parent_folders("env.hcl"))
}

inputs = {
  name = "alertmanager-748"
  environment = local.env_vars.locals.environment
  region = local.region_vars.locals.aws_region
  tags = {
    BusinessUnit = "operations"
    ServiceCategory = "security"
    Environment = local.env_vars.locals.environment
    ManagedBy = "terragrunt"
  }
}