include "root" {
  path = find_in_parent_folders()
}

dependencies {
  paths = [
    "../../../../../../../../eu-west-1/staging/finance/data/service-2/compute/fargate-726",
    "../../../../../../../../us-east-1/qa/finance/platform/service-2/compute/batch-381",
    "../../../../../../../../eu-west-1/qa/operations/data/service-3/storage/backup-532",
    "../../../../../../../../us-west-2/dev/finance/platform/service-1/monitoring/prometheus-574",
    "../../../../../../../../us-west-2/dev/marketing/application/service-2/compute/lambda-979",
  ]
}

terraform {
  source = "git::git@github.com/example/terraform-modules.git//direct-connect-221?ref=v1.0.0"
}

locals {
  account_vars = read_terragrunt_config(find_in_parent_folders("account.hcl"))
  region_vars  = read_terragrunt_config(find_in_parent_folders("region.hcl"))
  env_vars     = read_terragrunt_config(find_in_parent_folders("env.hcl"))
}

inputs = {
  name = "direct-connect-221"
  environment = local.env_vars.locals.environment
  region = local.region_vars.locals.aws_region
  tags = {
    BusinessUnit = "operations"
    ServiceCategory = "security"
    Environment = local.env_vars.locals.environment
    ManagedBy = "terragrunt"
  }
}