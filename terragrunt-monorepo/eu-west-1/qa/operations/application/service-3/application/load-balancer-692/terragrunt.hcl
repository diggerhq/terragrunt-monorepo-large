include "root" {
  path = find_in_parent_folders()
}

dependencies {
  paths = [
    "../../../../../../../../us-east-1/dev/marketing/application/service-3/application/cache-layer-205",
    "../../../../../../../../us-east-1/qa/finance/platform/service-1/network/transit-gateway-990",
    "../../../../../../../../eu-west-1/staging/finance/platform/service-2/compute/ecs-813",
    "../../../../../../../../us-east-1/dev/operations/data/service-2/compute/spot-fleet-635",
  ]
}

terraform {
  source = "git::git@github.com/example/terraform-modules.git//load-balancer-692?ref=v1.0.0"
}

locals {
  account_vars = read_terragrunt_config(find_in_parent_folders("account.hcl"))
  region_vars  = read_terragrunt_config(find_in_parent_folders("region.hcl"))
  env_vars     = read_terragrunt_config(find_in_parent_folders("env.hcl"))
}

inputs = {
  name = "load-balancer-692"
  environment = local.env_vars.locals.environment
  region = local.region_vars.locals.aws_region
  tags = {
    BusinessUnit = "finance"
    ServiceCategory = "platform"
    Environment = local.env_vars.locals.environment
    ManagedBy = "terragrunt"
  }
}